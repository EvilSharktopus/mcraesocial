...
def process_pptx(src: Path, course: str, unit: str, original_name: str = None):
    with _libreoffice_lock:
        _process_pptx_impl(src, course, unit, original_name)

def _process_pptx_impl(src: Path, course: str, unit: str, original_name: str = None):
    title = original_name or Path(src).stem
    slug  = slugify(title)
    dest  = slides_dest_dir(course, unit, slug)
    dest.mkdir(parents=True, exist_ok=True)
...
def dispatch(path: Path):
    ...
    if suffix == ".pptx":
        ...
        import uuid
        tmp_file = tmp_dir / f"convert_{uuid.uuid4().hex[:8]}.pptx"
